...
export default function WelcomePage() {
  return (
    <main className="flex min-h-screen flex-col">
      ...
      <div className="flex flex-1 flex-col px-7 pt-14 pb-8 lg:hidden">
...
function Brand({ size = "md" }: { size?: "sm" | "md" }) {
  const markSize = size === "sm" ? "size-10" : "size-9";
  const markText = size === "sm" ? "text-base" : "text-sm";
  const nameSize = "text-[13px]";
  return (
    <div className="flex items-center gap-2.5">
      <div
        className={
          "bg-primary text-primary-foreground font-display flex items-center justify-center rounded-xl font-bold tracking-tight " +
          markSize +
          " " +
          markText
        }
      ...
      <div
        className={
          "font-display font-semibold tracking-wider uppercase " + nameSize
        }
      >
        {t.app.name}
